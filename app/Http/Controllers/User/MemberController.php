...
class MemberController extends Controller
{
    ...
    {
        $users = DB::table('users')->select('users.id', 'users.avatar', 'users.last_name', 'users.first_name', 'users.middle_name', 'users.username', 'users.email', 'users.birthdate', 'users.agency', 'users.occupation', 'users.address', 'countries.country')->join('countries', 'users.country_id' , '=', 'countries.id')->where(['is_admin' => false])->orderBy('id', 'DESC')->paginate(10);

        if ($request->search != null || $request->sorting != null)
        {
            $users = DB::table('users')->select('users.id', 'users.avatar', 'users.last_name', 'users.first_name', 'users.middle_name', 'users.username', 'users.email', 'users.birthdate', 'users.agency', 'users.occupation', 'users.address', 'countries.country')->join('countries', 'users.country_id' , '=', 'countries.id')->where(['is_admin' => false])->where(function ($query) use ($request) {
